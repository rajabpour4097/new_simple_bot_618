            ...
            if process_data:
                ...
                if len(legs) > 2:
                    ...
                    if is_swing:
                        ...
                        if swing_type == 'bullish' and cache_data.iloc[-2]['close'] > legs[1]['start_value']:
                            ...
                            log(f"📈 New fibonacci created: fib1:{state.fib_levels['1.0']} time:{legs[2]['start']} - fib0.618:{state.fib_levels['0.618']} - fib0:{state.fib_levels['0.0']} time:{legs[2]['end']}", color='green')
                    ...
                    if state.fib_levels:
                        ...
                        if last_swing_type == 'bullish':
                            if cache_data.iloc[-2]['high'] > state.fib_levels['0.0']:
                                ...
                                # Should it be reset???
                                log(f"📈 Updated fibonacci: fib1:{state.fib_levels['1.0']} - fib0.618:{state.fib_levels['0.618']} - fib0:{state.fib_levels['0.0']}", color='green')
                            elif cache_data.iloc[-2]['low'] < state.fib_levels['1.0']:
                                state.reset()
                                log(f"📈 Price dropped below fib1 on bullish and reset fib levels", color='red')
                            elif cache_data.iloc[-2]['low'] <= state.fib_levels['0.618']:
                                log(f"📈 Price touched fib0.618 on bullish -- cache_data status is {cache_data.iloc[-2]['status']}", color='red')
                            ...
                            elif cache_data.iloc[-2]['high'] >= state.fib_levels['0.618']:
                                log(f"📉 Price touched fib0.618 on bearish -- cache_data status is {cache_data.iloc[-2]['status']}", color='red')
                                if not state.first_touch:
                                    state.first_touch_value = cache_data.iloc[-2]
                                    state.first_touch = True
                ...
                if last_swing_type == 'bullish' and state.second_touch:
                    ...
                    try:
                        ...
                        fib1_p = fib.get('1.0')
                        log(
                            f"ENTRY_CTX_BUY | fib0_time={state.fib0_time} value={fib0_p} | fib618={fib.get('0.618')} | fib09={fib.get('0.9')} | fib1_time={state.fib1_time} value={fib1_p}",
                            color='cyan'
                        )
                    except Exception:
                        pass
                    ...
                    try:
                        send_trade_email_async(
                            subject=f"NEW BUY ORDER {MT5_CONFIG['symbol']} 618",
                            body=(
                                f"Time: {datetime.now()}\n"
                    ...
                    if result and getattr(result, 'retcode', None) == 10009:
                        ...
                        try:
                            send_trade_email_async(
                                subject = f"Last order result 618",
                                body=(
                ...
                if last_swing_type == 'bearish' and state.second_touch:
                    ...
                    try:
                        ...
                        fib1_p = fib.get('1.0')
                        log(
                            f"ENTRY_CTX_SELL | fib0_time={state.fib0_time} value={fib0_p} | fib618={fib.get('0.618')} | fib09={fib.get('0.9')} | fib1_time={state.fib1_time} value={fib1_p}",
                            color='cyan'
                        )
                    except Exception:
                        pass
                    ...
                    try:
                        send_trade_email_async(
                            subject=f"NEW SELL ORDER {MT5_CONFIG['symbol']} 618",
                            body=(
                                f"Time: {datetime.now()}\n"
                                f"Symbol: {MT5_CONFIG['symbol']}\n"
